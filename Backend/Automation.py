...
from Backend.HomeAutomation import HomeAutomation
import os
import subprocess
import keyboard
from Backend.ppt import PPT
...
async def query(payload):
    ...
    "" if len(response.content)>1000 else print(response.content)
...
async def TranslateAndExecute(commands: list[str]):
    ...
    for command in commands:
        if command.startswith("open "):
            ...
            funcs.append(fun)
        elif command.startswith("home automation "):
            fun = asyncio.to_thread(HomeAutomation, command.removeprefix("home automation "))
            funcs.append(fun)
        elif command.startswith("ppt "):
            
            fun = asyncio.to_thread(PPT, command.removeprefix("ppt "))
            funcs.append(fun)
        else:
